...
def base(data, name="TPOT Regressor"):
    x, y = data
    x_train, x_test, y_train, y_test = train_test_split(
        x, y, train_size=0.80, test_size=0.20, shuffle=False
    )

    model = TPOTRegressor(generations=5, population_size=50, verbosity=2, n_jobs=-1)
...
def main():
    model_v1 = base(v1(), "TPOT Regressor - V1")
    model_v2 = base(v2(), "TPOT Regressor - V2")
